...
class RetroCard extends React.Component {
  ...
  currentDate(){
    ...
    let cardDateString = `${date < 10 ? `0${date}`:`${date}`}/${month < 10 ? `0${month}`:`${month}`}/${year}`
  ...
  currentTime() {
    ...
    let cardTimeString = `${hour < 10 ? `0${hour}`:`${hour}`}:${minute < 10 ? `0${minute}`:`${minute}`}:${second < 10 ? `0${second}`:`${second}`}`
      ...
      <div className="retroCard" style={{borderColor: cardColor}}>
        ...
        <div className="retro-card-options">
          ...
          <div className="retro-card-btns">
            ...
            <button className="retro-card-delete" onClick={onDeleteCard}><img src={trash} alt="trashIcon"/></button>
          </div>
        </div>
      </div>
    );
